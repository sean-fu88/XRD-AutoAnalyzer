...
def train_model(x_train, y_train, n_phases, num_epochs, is_pdf=False, n_dense=[3100, 1200], dropout_rate=0.7):
    """
    Args:
...
def main(xrd, num_epochs, testing_fraction, is_pdf = False, fmodel='Model.h5'):
    ...
    # Save model
    if is_pdf:
        model_filename = "pdf_" + fmodel
    else:
        model_filename= "xrd_" + fmodel
    model.save("Models/" + model_filename, include_optimizer=False)
